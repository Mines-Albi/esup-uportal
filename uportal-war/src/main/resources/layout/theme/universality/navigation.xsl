                      ...
                      <xsl:otherwise>
                          ...
                          <xsl:for-each select="//navigation/tab[@activeTab='true']/tabChannel">
                              <xsl:if test="not(@hideFromDesktop='true')">
                                  ...
                                  <li id="uPfname_{@fname}" class="portal-subnav {$SUBNAV_POSITION} {$SUBNAV_SELECTED}"> <!-- Each subnavigation menu item.  The unique ID can be used in the CSS to give each menu item a unique icon, color, or presentation. -->
                                      <xsl:variable name="portletSubNavLink">
                                          <xsl:choose>
                                              <!-- if canadian tab the render normal link -->
                                              <xsl:when test="$TAB_CANADIAN_MODE='true'">
